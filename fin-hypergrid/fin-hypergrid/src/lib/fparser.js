(function (root) {
  ...
  class Function {
    constructor(paramStr, funcName, parentFormula) {
      // Functions can have multiple params, comma separated.
      // Split them:
      this.name = funcName;
      this.args = this.splitFunctionParams(paramStr);
      this.parentFormula = this;

      if (parentFormula.rootFormula) {
        parentFormula.rootFormula.currentFunction = this;
      }
      else {
        parentFormula.currentFunction = this;
      }

      for (let i = 0; i < this.args.length; i++) {
        this.args[i] = new Formula(this.args[i], parentFormula);
      }
    ...
    splitFunctionParams(toSplit) {
      ...
      for (let i = 0; i < toSplit.length; i++) {
        if (toSplit[i] === ',' && pCount === 0) {
          ...
          paramStr += toSplit[i];
          if (pCount < 0) {
            console.log('ERROR: Too many closing parentheses!');
          }
        } else {
          paramStr += toSplit[i];
        }
      ...
      if (pCount !== 0) {
        console.log('ERROR: Too many opening parentheses!');
      }
  ...
  class Formula {
    constructor(fStr, topFormula = null) {
      ...
      this.rootFormula = topFormula ? (topFormula.rootFormula || topFormula) : null;

      this.formulaStr = fStr;
      // parsed state
      this.parsedState = 0;   //
      this.pCount = 0;        // opened parenthess count.
     ...
     *   - a string, e.g, "string"
     *   - a column identifier, e.g $[column name]
     *
        ...
        switch (state) {
          case 0:
            ...
            if (char.match(/[0-9.]/)) {
              ...
              if (act < str.length - 1) {
                str.charAt(act + 1);
                state = 'within-column-identifier';
                tmp = '';
              }
              ...
              state = 'within-func';
              if (act === lastChar) {
                expressions.push(this.createFunctionEvaluator('', tmp));
              }
              // else {
    ...
    createFunctionEvaluator(arg, fname) {
      ...
      return new Function(arg, fname, this);
