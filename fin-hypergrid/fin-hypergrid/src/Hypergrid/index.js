...
var Hypergrid = Base.extend('Hypergrid', {
    initialize: function(container, options) {
        ...
        document.addEventListener('mousedown', this.mouseCatcher = function() {
            if (this.cellEditor && !this.cellEditor.canStopEditing()) {
                return;
            }
            this.abortEditing();
        }.bind(this));
        ...
        //this.canvas.canvas.style["borderRadius"] = "4px 4px 0 0";
    },
    ...
    onUpdateColumnName: function(c, newValue){
      if (c && c.colDef){
        this.behavior.dataModel._viewer.set_column_dname(c.colDef.originalName, c.colDef.headerName, newValue);
      }
    },
    ...
    getClipboardCopyValue: async function() {
        if (this.hasFocus() || this.menuClick) {
            if (this.behavior.dataModel.copyFetch) {
                ...
                await this.behavior.dataModel.copyFetch({
                    start_row: selectionRect.origin.y,
                    end_row: selectionRect.origin.y + rowCount,
                    start_col: selectionRect.origin.x,
                    end_col: selectionRect.origin.x + colCount
                });
            }
            this.behavior.dataModel.setFetchCopyData(true);
    ...
    checkClipboardCopy: function(event) {
        if (this.hasFocus() || this.menuClick) {
            ...
            this.behavior.dataModel.resetCopyData();
            this.setCopyArea();
            this.repaint();
        }
    },
    ...
    beCursor: function(cursorName) {
        ...
        if (!this.isCursorProgress || cursorName.includes("progress")) {
    ...
    /**
     * @memberOf Hypergrid#
     * @desc Update begin and end for progress and Switch the cursor for grid
     * @param {*} cursorName
     * @param {*} isProgress
     */
    beProgressCursor: function(cursorName, isProgress = undefined) {
        this.isCursorProgress = isProgress;
        this.beCursor(cursorName);
    },
    ...
    getDataBounds: function() {
        ...
        var ratio = this.behavior.dataModel.getZoomRatio();
        return new Rectangle(0, 0, (b.origin.x + b.extent.x) * ratio, (b.origin.y + b.extent.y) * ratio);
    },
    ...
    editAt: async function(event) {
        ...
        const is_qfsort_clicked = this.behavior.dataModel.is_qfsort_clicked(event.dataCell.y, event.dataCell.x, event);

        if (!is_qfsort_clicked && event.isDataColumn && (cellEditor = await this.getCellEditorAt(event))) {
    ...
    set_multiple_col_width: function(arr_col_width) {
        if (this.abortEditing()) {
            this.behavior.set_multiple_col_width(arr_col_width);
        }
    },
