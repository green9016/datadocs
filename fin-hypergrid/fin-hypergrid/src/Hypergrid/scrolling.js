...
exports.mixin = {
    ...
    scrollToMakeVisible: function(c, r) {
        ...
        */
        var _renderer$dataWindow = this.renderer.dataBoundWindow || this.renderer.dataWindow,
        ...
        if (c >= fixedColumnCount) {
            ...
            if (delta >= 0) {
                pxDelta = 0;
                var maxCol = this.behavior.dataModel.getColumnsWithValuesCount();
                if (maxCol && (c == maxCol -1 || (c > maxCol -1 && c == 25))){
                  // Scroll to last column (Z or later)
                  if (c >= 25){
                    this.sbHScroller.index = this.sbHScroller.range.max;
                  }else{
                    this.sbHScroller.index = this.sbHScroller.range.max - (25 - c -1) * this.properties.defaultColumnWidth;
                  }
                }else{
    ...
    getVScrollbarTopMargin: function getVScrollbarTopMargin() {
        ...
        var res = void 0;
        var rowIndex = 0;//this.properties.fixedRowCount;
