...
var Behavior = Base.extend('Behavior', {
    ...
    clearColumns: function() {
        ...
        var treeColumnOld = undefined; //this.allColumns && this.allColumns[tc] || this.columns && this.columns[tc];
        var rowColumnOld = this.allColumns && this.allColumns[rc] || this.columns && this.columns[rc];
        ...
        this.allColumns[tc] = this.columns[tc] = this.newColumn(treeColumnOld && treeColumnOld.properties ? treeColumnOld.properties : {
            index: tc,
        ...
        this.allColumns[rc] = this.columns[rc] = this.newColumn(rowColumnOld && rowColumnOld.properties ? rowColumnOld.properties : {
            index: rc,
    ...
    setColumnWidth: function(columnOrIndex, width) {
        ...
        if (columnOrIndex && columnOrIndex.colDef){
          //let save_user_width = _.debounce(this.grid.api.save_user_width, 400).bind(this);
          //save_user_width(columnOrIndex.colDef.originalName, width);
          this.grid.api.save_user_width(columnOrIndex.colDef.originalName, width);
        }
    ...
    /*
    arr_col_width = [
      {original_name: "Header", index: 1, user_width: 250},
      {original_name: "HeaderA", index: 2, user_width: 500},
      {original_name: undefined, index: 3, user_width: 500},
    ]
    */
    set_multiple_col_width: function(arr_col_width) {
        ...
        for (var i = 0; i < arr_col_width.length; i++){
          var column = this.getActiveColumn(arr_col_width[i].index);
          column.setWidth(arr_col_width[i].user_width);
        }

        this.grid.api.save_user_width_to_multiple_cols(arr_col_width);

        this.stateChanged();

        this.grid.fireSyntheticOnColumnResizedEvent();
    },
    ...
    getCellEditorAt: function(event) {
        console.log('behavior getcelleditorat---------');
