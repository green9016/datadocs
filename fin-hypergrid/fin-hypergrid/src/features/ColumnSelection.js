...
var ColumnSelection = Feature.extend('ColumnSelection', {
    ...
    handleMouseDown: function(grid, event) {
        ...
        if (
            ...
            (event.isHeaderCell || event.isHeaderTreeCell)
            /*(
                grid.properties.autoSelectColumns ||
                (event.isHeaderCell || event.isHeaderTreeCell) && event.mousePoint.y >= 5
            )*/
        ) {
            ...
            this.dragging = true;
            if (!(grid.behavior.dataModel.isRowPivot() && event.dataCell && event.dataCell.x === 0)){
                this.cursor = GRAB;
            }
    ...
    handleMouseDragCellSelection: function(grid, x, keys) {
        ...
        /*var selectedColumns = grid.getSelectedColumnsGroup(x);
        var firstR = selectedColumns.first;
        var lastR = selectedColumns.last;
        grid.clearColumnSelection();
        for (var fi in firstR){
            if (firstR[fi].length > 0){
                grid.selectColumn(firstR[fi][0], firstR[fi][firstR[fi].length - 1]);
            }
        }
        if (lastR.length > 0){
            grid.selectColumn(lastR[0], lastR[lastR.length - 1]);
        }
        grid.setDragExtent(grid.newPoint(x, 0));*/
    ...
    extendSelection: function(grid, x, keys) {
        ...
        if (hasSHIFT && mouseX == -1){
            grid.setMouseDown(grid.newPoint(x, 0));
            mouseX = grid.getMouseDown().x;
        }

        if (x < -1) { // outside of the grid?
            return; // do nothing
        }

        if (hasSHIFT) {
            grid.clearMostRecentColumnSelection();
            grid.selectColumn(x, mouseX, hasSHIFT);
            grid.setDragExtent(grid.newPoint(x - mouseX, 0));
        }else if(hasCTRL){
            /*var selectedCs = grid.getSelectedColumns() || [];
            selectedCs.push(x);
    ...
    handleRIGHT: function(grid) {
        console.log('---col---handle right------');

        this.moveSingleSelect(grid, 1);
    },

    handle0: function(grid, detail) {
        if (!detail || !detail.primitiveEvent){
            return;
        }

        if ((detail.primitiveEvent.ctrlKey && detail.primitiveEvent.keyCode === 48)
            || (detail.primitiveEvent.metaKey && detail.primitiveEvent.keyCode === 48)) {
            /*
            var colDef = grid.columnDefs;
            var selected_columns = grid.getSelectedColumns() || [];

            var columns = grid.behavior.columns;

            var c_names = selected_columns.map(function(i){
            ...
            c_names.forEach(function(c_name){
                grid.getColDefs(c_name).forEach(function (singleColDef) {
            ...
            grid.behavior.dataModel._viewer.did_inactive_columns(co_names);
            */
