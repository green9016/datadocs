    ...
    _createClass(HyperList, [
        ...
        },
        {
            ...
            value: function refresh(element, userProvidedConfig) {
                ...
                if (typeof config.useFragment !== "boolean") {
                    this._config.useFragment = false;//true;
                }
        ...
        },
        {
            ...
            value: function _renderChunk(force) {
                ...
                //*
                // Add & Remove only changed ones.
                var oldNodes = element.childNodes;
                var newNodes = fragment;//.childNodes;
                var start = 0;
                var end = 0;
                ...
                for (var i = 0; i < newNodes.length; i++) {
                    ...
                    // Check if newNode is equal to oldNode. (not same, but equal)
                    var isFound = false;
                    for (var j = start; j < oldNodes.length; j++) {
                        if (oldNodes[j].isEqualNode(newNode)) {
                            isFound = true;
                            end = j;
                            break;
                        }
                    ...
                    if (isFound) {
                        ...
                        for (var j = end - 1; j >= start; j--) {
                            element.removeChild(oldNodes[j]);
                        }
                    }

                    // if not found, insert a new node
                    if (!isFound) {
                        if (start == 0 && oldNodes.length > 0) {
                            element.insertBefore(newNode, element.firstChild);
                        } else if (start >= oldNodes.length) {
                            element.appendChild(newNode);
                        } else {
                            element.insertBefore(newNode, oldNodes[start]);
                        }
                    }
                ...
                // Remove the rest in the case of oldNodes.length > newNodes.lengh
                for (var i = oldNodes.length-1; i >= newLength; i--) {
