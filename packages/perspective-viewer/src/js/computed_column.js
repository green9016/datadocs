...
export const COMPUTATIONS = {
    copy_value_from: new Computation("copy_value_from", "any", "any", x => x)
...
class ComputedColumn extends HTMLElement {
    ...
    _parse_data_transfer(data) {
        ...
        return {
            column_name: column_data[5],
            column_type: column_data[3],
            agg_name: column_data[4]
        };
    }
    ...
    _drop_column(event) {
        ...
        if (is_swap) {
            ...
            this._set_input_column(event, current_column_name, current_column_type, data.agg_name);
        ...
        this._set_input_column(event, data.column_name, data.column_type, data.agg_name);
    }
    ...
    _apply_state(columns, computation, name) {
        ...
        this._column_name_input.innerText = name || this._get_calculated_field();
    ...
    _auto_column_name() {
        ...
        if (this.state.computation.name === "copy_value_from") {
            this._column_name_input.innerText = this._get_calculated_field();
        } else {
    ...
    _get_calculated_field() {
        var calculated_name = "calculated field";
        var index = 0;
        while(this.column_names.includes(calculated_name)) {
            index++;
            calculated_name = "calculated field " + index;
        }
    ...
    _clear_column_name() {
        const input = this._column_name_input;
        input.innerText = this._get_calculated_field();
        this.state["name_edited"] = false;
        this._set_column_name();
    }

    // input column
    _set_input_column(event, name, type, agg_name) {
        const computation = this.state.computation;
        ...
        if (
            (computation_type !== "any" && computation_type !== "float" && computation_type !== "datetime" && type !== computation_type) ||
            (computation_type !== "any" && computation_type === "float" && type !== "float" && type !== "integer") ||
            (computation_type !== "any" && computation_type === "datetime" && type !== "datetime" && type !== "date") ||
            (agg_name === "custom")
        ) {
            this._register_inputs();
        ...
        if (computation_type == "any") {
            this._update_return_type(type);
        }
    ...
    _update_return_type(return_type) {
    ...
    _update_rename_for_list(rename_columns) {
        if (!rename_columns) {
            return;
        }
        for (let child of this._input_columns.children) {
            var rows = Array.prototype.slice.call(child.getElementsByTagName("perspective-row"));
            if (rows && rows.length > 0) {
                rows.map(x => {
                    let name = x.getAttribute("name");
                    if (rename_columns[name]) {
                        x.setAttribute("name", rename_columns[name]);
                        x.setAttribute("new_base_name", rename_columns[name]);
                    }
                });
            }
        }
        this.state.input_columns = this.state.input_columns.map(x => {
            if (x && rename_columns[x.name]) {
                x.name = rename_columns[x.name];
            }
            return x;
        });
        this._apply_state(this.state.input_columns, this.state.computation);
    ...
    _register_callbacks() {
        ...
        this._column_name_input.addEventListener("keyup", event => {
            console.log("_column_name_input_keyup-----")
