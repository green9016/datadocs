...
import {clean_tokens, Comma, ColumnName, StringValue, NumberValue, As, Whitespace, LeftParen, RightParen, OperatorTokenType, FunctionTokenType, UpperLowerCaseTokenType} from "./lexer";
...
export class ComputedExpressionAutocompleteSuggestion {
     ...
     */
    constructor({label, value, pattern, signature, help, input_types, return_type, num_params, params, is_column_name} = {}) {
        this.label = label;
        this.value = value;
...
class ComputedColumnParser {
    constructor() {
        this.is_initialized = false;
        this._vocabulary = {};
        this._tokens = [Whitespace, Comma, As, ColumnName, StringValue, NumberValue, LeftParen, RightParen];
        this._metadata;
        this._lexer;
        this._parser;
        this._visitor;
    ...
    init(metadata) {
        ...
        this._visitor = this.createVisitor();
        console.log('---lexer--');
    ...
    createLexer() {
        ...
        for (const key in this._metadata) {
            ...
            const token = this.makeToken(meta);
...
console.log('---lexer--1---');
    ...
    makeToken(meta) {
        ...
        token.params = meta.params;

        return token;
    }
    
    createParser() {
        return new ExpressionParser(this._vocabulary);
    }
    ...
    createVisitor() {
        ...
        class ExpressionColumnVisitor extends base_visitor {
            constructor() {
                super();
            ...
            _VisitOperatorComputedColumn(ctx, computed_columns, get_operator) {
                ...
                if (ctx.right) {
                    ...
                    ctx.right.forEach((rhs, idx) => {
                        ...
                        const column_name = as ? as : COMPUTED_FUNCTION_FORMATTERS[operator](left_hand.value, right.value);

                        computed_columns.push({
                            column: column_name,
                            computed_function_name: operator,
                            inputs: [left_hand.value, right.value],
                            input_types: [left_hand.type, right.type]
                        });

                        console.log('-------computed_columns---1----', computed_columns);
            ...
            FunctionComputedColumn(ctx, computed_columns) {
                ...
                const computed = {
                    ...
                    inputs: input_columns.map(v => v.value),
                    input_types: input_columns.map(v => v.type)
                };

                computed_columns.push(computed);
                console.log('-------computed_columns---2----', computed_columns);
            ...
            ColumnName(ctx, computed_columns) {
                ...
                if (ctx.ParentheticalExpression) {
                    ...
                    return {value: ctx.stringValue[0].payload, type: "string"};
                } else if (ctx.numberValue) {
                    return {value: ctx.numberValue[0].image, type: "number"};
                } else {
                    return {value: ctx.columnName[0].payload, type: "column"};
                }
            }
        ...
        return new ExpressionColumnVisitor();
    }
    ...
    get_param_idx_of_func(token, lexer_result) {
        if (!token || !lexer_result) {
            return 0;
        }
        ...
        for (i = i + 1; i < tokens.length; i++) {
            if (tokenMatcher(tokens[i], LeftParen)) {
                paramIdx = 0;
                parenCount++;
            }
            else if (tokenMatcher(tokens[i], RightParen)) {
                parenCount--;
            ...
            else if  (tokenMatcher(tokens[i],  Comma)) {
                if (parenCount == 1) {
                    paramIdx++;
                }
            }
...
export const COMPUTED_EXPRESSION_PARSER = new ComputedColumnParser();
