...
export const NumberTokenType = createToken({
    name: "NumberTokenType",
    pattern: Lexer.NA
});

export const StringTokenType = createToken({
    name: "StringTokenType",
    pattern: Lexer.NA
});
// Create tokens for column names and computed function names
export const COLUMN_NAME_REGEX_PATTERN = /(\[)(.*?[^\\])(\])/y;
export const STRING_REGEX_PATTERN = /(["'])(.*?[^\\])\1/y;
...
const match_string = function(string, start_offset) {
    STRING_REGEX_PATTERN.lastIndex = start_offset;
    const result = STRING_REGEX_PATTERN.exec(string);
...
export const StringValue = createToken({
    name: "stringValue",
    pattern: {exec: match_string},
    line_breaks: false,
    categories: [StringTokenType]
});
export const NumberValue = createToken({
    name: "numberValue",
    pattern: /-?\d+\.?\d*/,
    line_breaks: false,
    categories: [NumberTokenType]
});
