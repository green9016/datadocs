    ...
    describe.page(
        ...
        () => {
            ...
            test.skip("aggregates by computed column.", async page => {
                await add_computed_column(page);
                const viewer = await page.$("perspective-viewer");
                await page.evaluate(element => element.setAttribute("row-pivots", '[{"n":"Quantity"}]'), viewer);
            ...
            test.capture("pivots by computed column.", async page => {
                await add_computed_column(page);
                const viewer = await page.$("perspective-viewer");
                await page.evaluate(element => element.setAttribute("row-pivots", '[{"n": "new_cc"}]'), viewer);
            });
