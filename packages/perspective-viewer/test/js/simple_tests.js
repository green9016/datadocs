...
exports.default = function() {
    ...
    test.capture("pivots by a row.", async page => {
        ...
        await page.evaluate(element => element.setAttribute("row-pivots", '[{"n": "State"}]'), viewer);
    });

    test.capture("pivots by two rows.", async page => {
        const viewer = await page.$("perspective-viewer");
        await page.shadow_click("perspective-viewer", "#config_button");
        await page.evaluate(element => element.setAttribute("row-pivots", '[{"n": "Category"},{"n" :"Sub-Category"}]'), viewer);
    });

    test.capture("pivots by a column.", async page => {
        const viewer = await page.$("perspective-viewer");
        await page.shadow_click("perspective-viewer", "#config_button");
        await page.evaluate(element => element.setAttribute("column-pivots", '[{"n": "Category"}]'), viewer);
    });
    ...
    test.capture("pivots by two rows and two columns.", async page => {
        const viewer = await page.$("perspective-viewer");
        await page.shadow_click("perspective-viewer", "#config_button");
        await page.evaluate(element => element.setAttribute("row-pivots", '[{"n": "Region"},{"n": "State"}]'), viewer);
        await page.waitForSelector("perspective-viewer:not([updating])");
        await page.evaluate(element => element.setAttribute("column-pivots", '[{"n": "Category"},{"n": "Sub-Category"}]'), viewer);
    });
