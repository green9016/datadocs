...
{
    t_table tbl(t_schema({"a", "b"}, {DTYPE_INT64, DTYPE_FLOAT64}, {DATA_FORMAT_NUMBER, DATA_FORMAT_NUMBER}), 5);
    tbl.init();
    tbl.reserve(5);
}
...
TEST(GNODE, explicit_pkey)
{
    ...
    options.m_port_schema = t_schema{{"x"}, {DTYPE_INT64}, {DATA_FORMAT_NUMBER}};
#ifndef WIN32
...
TEST(GNODE, implicit_pkey)
{
    ...
    options.m_port_schema = t_schema{
            {"psp_op", "psp_pkey", "x"}, {DTYPE_UINT8, DTYPE_INT64, DTYPE_INT64}, {DATA_FORMAT_NUMBER, DATA_FORMAT_NUMBER, DATA_FORMAT_NUMBER}};
...
{
    t_schema sch{{"p", "a"}, {DTYPE_INT64, DTYPE_INT64}, {DATA_FORMAT_NUMBER, DATA_FORMAT_NUMBER}};
    t_table tbl(sch, {{1_ns, 1_ts}, {1_ts, 1_ts}});
    t_config cfg{{"p"}, {"sum_a", AGGTYPE_SUM, "a"}};
    auto ctx = do_pivot<t_ctx1, std::int32_t, DTYPE_INT32, DATA_FORMAT_NUMBER>(
...
{
    t_schema sch{{"b", "s", "i"}, {DTYPE_BOOL, DTYPE_STR, DTYPE_INT64}, {DATA_FORMAT_TRUE_FALSE, DATA_FORMAT_TEXT, DATA_FORMAT_NUMBER}};
...
{
    t_schema sch{{"i", "s", "b"}, {DTYPE_BOOL, DTYPE_STR, DTYPE_INT64}, {DATA_FORMAT_TRUE_FALSE, DATA_FORMAT_TEXT, DATA_FORMAT_NUMBER}};
    auto t = mktscalar<bool>(true);
    auto f = mktscalar<bool>(false);
    t_table tbl(sch,
        {{t, "a"_ts, 1_ts}, {f, "b"_ts, 2_ts}, {t, "c"_ts, 3_ts},
...
TEST(SCALAR, scalar_repr)
{
    ...
    EXPECT_EQ(mktscalar<t_date>(t_date(0)).repr(), "date:v:Jan 1, 1900");
    EXPECT_EQ(mktscalar<t_duration>(t_duration(0)).repr(), "duration:v:00:00:00");
    EXPECT_EQ(
        mktscalar<t_time>(t_time(0)).repr(), "time:v:1900-01-01 00:00:00");
    EXPECT_EQ(
        mktscalar<t_decimal>(t_decimal("")).repr(), "decimal:v:");
    ...
    EXPECT_EQ(mknull(DTYPE_DURATION).repr(), "duration:i:null");
...
public:
    ...
    get_config()
    {
        return t_config{{"a"}, {"b"}, {{"sum_x", AGGTYPE_SUM, "x"}}, {{"a", SEARCHTYPE_EQUALS}}, {},
...
public:
    ...
    get_config()
    {
        return t_config{{"a", "b", "c"}, FILTER_OP_AND, {}, {}, {{"a", SEARCHTYPE_EQUALS}}, {}};
    }
};
...
TEST(IS_FLOATING_POINT, test_1)
{
    ...
    EXPECT_TRUE(perspective::is_floating_point(DTYPE_DECIMAL));
...
TEST(TYPE_TO_DTYPE, test_time)
{
    ...
    EXPECT_EQ(type_to_dtype<t_duration>(), DTYPE_DURATION);
...
TEST(GNODE_TEST, get_registered_contexts)
{
    ...
    auto ctx2
        = t_ctx2::build(sch, t_config({"s"}, {"i"}, {{AGGTYPE_SUM, "i"}}, {{"a", SEARCHTYPE_EQUALS}}, {}));
