...
public:
    friend class perspective::t_table;
    ...
    void set_dtype(t_dtype dtype);

    t_dataformattype get_data_format_type() const;

    void set_data_format_type(t_dataformattype dftype);
    ...
    void set_error(t_uindex idx, t_cell_error error);

    void set_valid(t_uindex idx, bool valid);

    void set_error_status(t_uindex idx);

    void set_warning_status(t_uindex idx);
    ...
    void set_truncated(t_warning_type truncated);

    const t_lstore& data_lstore() const;

    t_uindex size() const;

    std::shared_ptr<t_vocab> get_vocab() { return m_vocab; }
    std::shared_ptr<const t_vocab> get_vocab() const { return m_vocab; }
    ...
    t_tscalar get_scalar(t_uindex idx, bool include_error = false) const;
    t_tscalar get_unnest_scalar(t_uindex idx, t_uindex uidx, t_agg_level_type agg_level, t_binning_info binning,
        t_dataformattype df_type, std::vector<double>& dbinning) const;
    t_tscalar get_pivot_scalar(t_uindex idx) const;
    t_cell_error get_error(t_uindex idx) const;
    std::string get_error_message(t_uindex idx) const;
    ...
    bool is_error(t_uindex idx) const;

    bool is_warning(t_uindex idx) const;
    ...
    std::vector<double> get_column_binning_params() const;

    t_warning_type get_truncated() const;
...
private:
	...
	std::shared_ptr<t_column_meta> m_meta;

    std::shared_ptr<std::map<t_uindex, t_cell_error>> m_errors;

    t_warning_type m_truncated;
};
