...
struct PERSPECTIVE_EXPORT t_stnode {
    t_stnode(t_uindex idx, t_uindex pidx, const t_tscalar& value, std::uint8_t depth,
        const t_tscalar& sort_value, t_uindex nstrands, t_uindex aggidx, bool show = true);

    t_stnode();

    void set_nstrands(t_index nstrands);
    void set_sort_value(t_tscalar sv);
    void set_value(t_tscalar v);
...
struct t_cellinfo {
    t_cellinfo();
    t_cellinfo(t_index idx, t_depth treenum, t_index agg_index, t_uindex ridx, t_uindex cidx);
