...
template <typename DATA_T>
struct t_argsort_asc_cmp {
    t_argsort_asc_cmp(const DATA_T* base)
        : m_base(base) {}
...
template <typename DATA_T>
inline void
argsortasc(const DATA_T* b, std::vector<t_uindex>& output) {
    // Output should be the same size is erange-brange
    for (t_index i = 0, loop_end = output.size(); i != loop_end; ++i) {
        output[i] = i;
    }
    t_argsort_asc_cmp<DATA_T> cmp(const_cast<DATA_T*>(b));
    std::sort(output.begin(), output.end(), cmp);
}

template <typename DATA_T>
struct t_argsort_desc_cmp {
    t_argsort_desc_cmp(const DATA_T* base)
        : m_base(base) {}
...
template <typename DATA_T>
inline void
argsortdesc(const DATA_T* b, std::vector<t_uindex>& output) {
    // Output should be the same size is erange-brange
    for (t_index i = 0, loop_end = output.size(); i != loop_end; ++i) {
        output[i] = i;
    }
    t_argsort_desc_cmp<DATA_T> cmp(const_cast<DATA_T*>(b));
    std::sort(output.begin(), output.end(), cmp);
}

inline void
partition(const t_column* PSP_RESTRICT data_, t_column* PSP_RESTRICT leaves_, t_uindex bidx,
    t_uindex eidx, std::vector<t_chunk_value_span<t_tscalar>>& out_spans, t_sorttype sort_type = SORTTYPE_ASCENDING) {
    ...
    switch (nelems) {
        ...
        case 1: {
            ...
            t_cvs& c = out_spans[0];
            fill_chunk_value_span<t_tscalar>(c, data_->get_pivot_scalar(leaves[bidx]), bidx, eidx);
        } break;
        default: {
            std::vector<t_tscalar> buf(nelems);
            for (t_uindex idx = 0; idx < nelems; ++idx) {
                buf[idx] = data_->get_pivot_scalar(leaves[bidx + idx]);
            }

            std::vector<t_uindex> order(nelems);

            // Handle sort asc or desc in case have sort
            t_tscalar* buf_addr = &buf[0];
            if (sort_type != SORTTYPE_DESCENDING) {
                argsortasc(buf_addr, order);
            } else {
                argsortdesc(buf_addr, order);
            }
