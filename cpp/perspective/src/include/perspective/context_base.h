...
t_ctxbase<DERIVED_T>::t_ctxbase(const t_schema& schema, const t_config& config)
    ...
    , m_init(false) {
    m_features = std::vector<bool>(CTX_FEAT_LAST_FEATURE);
    if (config.get_pivots().size() == 0) {
        m_schema.update_data_formats(config.get_data_formats());
    }
    m_features[CTX_FEAT_ENABLED] = true;
}
...
template <typename DERIVED_T>
void
t_ctxbase<DERIVED_T>::update_data_formats() {
    if (m_config.get_data_formats().empty()) {
        return;
    }
    std::shared_ptr<t_table> stbl = m_state->get_table();
    if (stbl != nullptr) {
        t_table* tbl = stbl.get();
        tbl->update_data_formats(m_config.get_data_formats());
    }
}
