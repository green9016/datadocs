 ...
 * - a vector of strings containing the names of input column types
 * - a `t_computation` containing the input data types and return type
 *
 */
typedef std::tuple<std::string, t_computed_function_name, std::vector<std::string>, std::vector<std::string>, t_computation> t_computed_column_definition;


class PERSPECTIVE_EXPORT t_computed_column_input {
public:
    void        setScalar(const std::string& vale, const std::string& type);
    void        setColumn(std::shared_ptr<t_column> column);

    t_tscalar   getValue(int idx) const;

    std::shared_ptr<t_column>   m_column;
    t_tscalar                   m_scalar;
    std::string                 m_type;     // string, number, column, boolean, ...
};
