...
public:
    ...
    t_data_slice(std::shared_ptr<CTX_T> ctx, t_uindex start_row, t_uindex end_row,
        ...
        std::vector<std::vector<t_tscalar>> map_columns = {});
    ...
    std::vector<std::int8_t> get_row_path_extra(t_uindex ridx) const;

    std::vector<t_tscalar> get_row_header(t_uindex ridx) const;
    ...
    const std::vector<std::vector<t_tscalar>>& get_short_column_names() const;
...
private:
    ...
    std::vector<std::vector<t_tscalar>> m_short_column_names;
