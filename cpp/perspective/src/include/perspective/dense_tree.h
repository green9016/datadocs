...
public:
    ...
    t_dtree(const std::string& dirname, t_dssptr ds, const std::vector<t_pivot>& pivots,
    ...
    void check_pivot(const t_filter& f, t_uindex level, t_config& config, t_mask &dt_msk, const std::vector<t_sortspec>& ctx_sortby = {});
    void pivot(const t_filter& f, t_uindex level, t_config& config, t_mask &dt_msk, const std::vector<t_sortspec>& ctx_sortby = {});

    t_uindex size() const;

    t_tscalar _get_value(const t_filter& filter, t_index nidx, bool sort_value) const;
