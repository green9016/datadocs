...
public:
    ...
    t_table* get_table();
    std::shared_ptr<t_table> get_table_sptr();
    const t_table* get_const_table() const;

    void pprint() const;
    std::vector<std::string> get_registered_contexts() const;
    t_schema get_tblschema() const;
    void rename_tbl_columns(const std::map<std::string, std::string> col_map);
    ...
    const std::vector<t_custom_column>& get_custom_columns() const;
    ...
    const std::map<std::string, std::string>& set_dname_mappings(const std::vector<std::string>& colnames, const std::vector<std::string>& dnames);
    const std::map<std::string, double> get_default_binning(const std::string& colname) const;
...
t_gnode::update_context_from_state(CTX_T* ctx, const t_table& flattened) {
    ...
    if (ctx->get_config().get_cancel_query_status()) {
        return;
    }
