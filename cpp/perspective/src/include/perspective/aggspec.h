...
struct PERSPECTIVE_EXPORT t_col_name_type {
    t_col_name_type();
    t_col_name_type(const std::string& name, t_dtype type, t_dataformattype dftype);
    std::string m_name;
    t_dtype m_type;
    t_dataformattype m_dftype;
};
...
public:
    ...
    std::vector<t_col_name_type> get_formula_output_specs(const t_schema& schema, std::string tbl_colname) const;
    bool can_change_data_format() const;
    std::vector<t_col_name_type> mk_col_name_type_vec(
        const std::string& name, t_dtype dtype, t_dataformattype dftype) const;
...
PERSPECTIVE_EXPORT t_dtype to_list_type(t_dtype coltype);
