...
cmp_mselem(const t_mselem& a, const t_mselem& b, const std::vector<t_sorttype>& sort_order,
    ...
    for (int idx = 0, loop_end = sort_order.size(); idx < loop_end; ++idx) {
        ...
        if (order != SORTTYPE_NONE) {
            if (first.get_dtype() == DTYPE_NONE || first.m_status == STATUS_INVALID || first.m_status == STATUS_ERROR) {
                return false;
            } else if (second.get_dtype() == DTYPE_NONE || second.m_status == STATUS_INVALID || second.m_status == STATUS_ERROR) {
                return true;
            }
        }
