...
struct t_table_index {
    std::vector<t_sortspec> sortby;
    std::vector<t_fterm> filters;
    std::vector<t_sterm> searchs;
    std::vector<t_index> v;
};
...
struct PERSPECTIVE_EXPORT t_table_recipe {
    ...
    std::vector<t_column_recipe> m_columns;
};
...
public:
    ...
    t_schema get_computed_schema(std::vector<t_computed_column_definition> computed_columns) const;
    t_uindex size() const;
    ...
    std::shared_ptr<t_column> get_column(t_uindex idx);
    ...
    std::shared_ptr<t_table> flatten() const;

    bool is_pkey_table() const;
    bool is_same_shape(t_table& tbl) const;
    ...
    t_mask filter_cpp(t_filter_op combiner, const std::vector<t_fterm>& fops, t_config& config) const;
    t_mask having_cpp(t_filter_op combiner, const std::vector<t_fterm>& fops, t_config& config,
        t_uindex level, std::map<t_uindex, t_depth> dmap) const;
    t_mask search_cpp(std::vector<t_searchspec> search_types, const std::vector<t_sterm>& sterms_, t_config& config) const;
    ...
    std::shared_ptr<t_column> add_column_sptr(const std::string& cname, t_dtype dtype, bool status_enabled, t_dataformattype datatype);
    t_column* add_column(const std::string& cname, t_dtype dtype, bool status_enabled, t_dataformattype datatype);
    void remove_column(const std::string& name);
    void promote_column(
        const std::string& cname, t_dtype new_dtype, t_dataformattype new_dftype, std::int32_t iter_limit, bool fill);
    ...
    void set_dname_mapping(const std::map<std::string, std::string>& dname_mapping);
    void clear_dname_mapping(const std::string& colname);
    void update_dname_mapping(const std::string& current_name, const std::string& new_name);
    const std::map<std::string, std::string>& get_dname_mapping() const;
    std::map<std::string, t_uindex> get_truncated_columns() const;

    std::shared_ptr<t_table_index> get_last_index() const { return m_last_index; }
    void set_last_index(const std::shared_ptr<t_table_index> &p) const { m_last_index = p; }
...
private:
    ...
    std::map<std::string, std::string> m_dname_mapping;
    t_table_recipe m_recipe;
    bool m_from_recipe;
    mutable std::shared_ptr<t_table_index> m_last_index;
};
...
t_table::flatten_body(FLATTENED_T flattened) const {
    ...
    switch (get_const_column("psp_pkey")->get_dtype()) {
        ...
        case DTYPE_LIST_STR: {
            flatten_helper_1<FLATTENED_T, std::vector<std::string>>(flattened);
        } break;
        case DTYPE_LIST_BOOL: {
            flatten_helper_1<FLATTENED_T, std::vector<bool>>(flattened);
        } break;
        case DTYPE_LIST_FLOAT64: {
            flatten_helper_1<FLATTENED_T, std::vector<double>>(flattened);
        } break;
        case DTYPE_LIST_INT64: {
            flatten_helper_1<FLATTENED_T, std::vector<std::int64_t>>(flattened);
        } break;
        case DTYPE_LIST_DATE: {
            flatten_helper_1<FLATTENED_T, std::vector<t_date>>(flattened);
        } break;
        case DTYPE_LIST_TIME: {
            flatten_helper_1<FLATTENED_T, std::vector<t_time>>(flattened);
        } break;
        case DTYPE_LIST_DURATION: {
            flatten_helper_1<FLATTENED_T, std::vector<t_duration>>(flattened);
        } break;
        case DTYPE_DECIMAL: {
            flatten_helper_1<FLATTENED_T, t_decimal>(flattened);
        } break;
    ...
    for (t_uindex colidx = 0; colidx < ndata_cols; ++colidx)
#endif
        {
            ...
            switch (scol->get_dtype()) {
                ...
                case DTYPE_LIST_STR: {
                    this->flatten_helper_2<std::vector<std::string>, t_rpvec>(sorted, fltrecs, scol, dcol);
                } break;
                case DTYPE_LIST_BOOL: {
                    this->flatten_helper_2<std::vector<bool>, t_rpvec>(sorted, fltrecs, scol, dcol);
                } break;
                case DTYPE_LIST_FLOAT64: {
                    this->flatten_helper_2<std::vector<double>, t_rpvec>(sorted, fltrecs, scol, dcol);
                } break;
                case DTYPE_LIST_INT64: {
                    this->flatten_helper_2<std::vector<std::int64_t>, t_rpvec>(sorted, fltrecs, scol, dcol);
                } break;
                case DTYPE_LIST_DATE: {
                    this->flatten_helper_2<std::vector<t_date>, t_rpvec>(sorted, fltrecs, scol, dcol);
                } break;
                case DTYPE_LIST_TIME: {
                    this->flatten_helper_2<std::vector<t_time>, t_rpvec>(sorted, fltrecs, scol, dcol);
                } break;
                case DTYPE_LIST_DURATION: {
                    this->flatten_helper_2<std::vector<t_duration>, t_rpvec>(sorted, fltrecs, scol, dcol);
                } break;
                case DTYPE_DECIMAL: {
                    this->flatten_helper_2<t_decimal, t_rpvec>(sorted, fltrecs, scol, dcol);
                } break;
