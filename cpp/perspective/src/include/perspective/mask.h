...
public:
    ...
    bool get(t_uindex idx) const { return m_bitmap.test(idx); }
    ...
    t_uindex size() const { return m_bitmap.size(); }
    void reserve(t_uindex sz) { m_bitmap.reserve(sz); }
    void append(uint32_t block) { m_bitmap.append(block); }
    void pprint() const;
