...
public:
    ...
    void get_col_index_map(std::vector<t_index>& out_data) const;

    void get_column_aggregate_info(std::vector<std::pair<t_index, t_index>>& out_data, t_depth v_depth, t_uindex num_aggs);

    void get_row_indices_vector(std::vector<t_indiceinfo>& out_data, t_depth v_depth, t_uindex num_aggs,
        std::map<t_depth, bool> subottal_map, bool flat_mode = false) const;
...
t_traversal::sort_by(const t_config& config, const std::vector<t_sortspec>& sortby,
    ...
    //std::vector<t_index> remove_nodes;
    ...
    while (!queue.empty()) {
        ...
        if (!h_children.empty()) {
            ...
            auto it = sort_map.find(head.m_depth);
            //auto limit = t_index(-1);
            if (it == sort_map.end()) {
                ...
                for (t_uindex i = 0, loop_end = n_changed; i < loop_end; i++) {
                    ...
                    src.get_aggregates_for_sorting(
                        children_ptidx[i], sortby_agg_indices, aggregates, ctx2, subtotal_indices);
            ...
            if (ndesc == nchild) {
                ...
                for (t_index idx = bidx; idx < eidx; idx++) {
                    ...
                    /*if (h_ctvidx == 0 && limit != -1 && limit <= idx - bidx) {
                        remove_nodes.push_back(idx);
                    }*/
                }
            } else {
                ...
                for (t_uindex idx = 0, loop_end = h_children.size(); idx < loop_end; idx++) {
                    ...
                    /*if (h_ctvidx == 0 && limit != -1 && limit <= idx) {
                        remove_nodes.push_back(c_ntvidx);
                    }*/
    ...
    // Remove node for limit
    /*for (t_index idx = remove_nodes.size() - 1; idx >= 0; --idx) {
        remove_subtree(remove_nodes[idx]);
    }*/
}
