...
t_sortspec::t_sortspec(t_index agg_index, t_sorttype sort_type, t_index sortby_index, t_index subtotal_index,
    t_index limit, t_limit_type limit_type)
    : m_agg_index(agg_index)
    , m_sort_type(sort_type)
    , m_sortby_index(sortby_index)
...
operator<<(std::ostream& os, const perspective::t_sortspec& t) {
    ...
    os << "t_sortspec<idx: " << t.m_agg_index << " stype: " << t.m_sort_type << " ssortby_index: " << t.m_sortby_index
        << " ssubtotal_index: " << t.m_subtotal_index << " slimit: " << t.m_limit << " slimit_type: " << t.m_limit_type << ">";
    return os;
}
