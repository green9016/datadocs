...
t_ctx_grouped_pkey::get_selection_summarize(std::vector<t_selection_info> selections) const {
    return std::map<std::pair<t_uindex, t_uindex>, t_tscalar>{};
}
...
t_ctx_grouped_pkey::get_aggregates_for_sorting(t_uindex nidx,
    ...
    t_ctx2*, const std::vector<t_index>& subtotal_indices) const {
...
std::vector<t_tscalar>
t_ctx_grouped_pkey::unity_get_row_header(t_uindex idx) const {
    return std::vector<t_tscalar>();
}
...
bool
t_ctx_grouped_pkey::get_show_leave() const {
    return true;
}
