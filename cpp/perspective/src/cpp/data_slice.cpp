...
t_data_slice<CTX_T>::t_data_slice(std::shared_ptr<CTX_T> ctx, t_uindex start_row,
    ...
    std::vector<std::vector<t_tscalar>> column_names, std::vector<std::vector<t_tscalar>> short_column_names)
    : m_ctx(ctx)
    ...
    , m_short_column_names(short_column_names) {
...
t_data_slice<CTX_T>::t_data_slice(std::shared_ptr<CTX_T> ctx, t_uindex start_row,
    ...
    std::vector<std::vector<t_tscalar>> short_column_names)
    : m_ctx(ctx)
    , m_start_row(start_row)
    , m_end_row(end_row)
    , m_start_col(start_col)
...
t_data_slice<CTX_T>::get_row_path(t_uindex ridx) const {
    ...
    auto paginationspec = config.get_paginationspec();
    ...
    if (paginationspec.enable_pagination()) {
        ...
        ridx += (page_num - 1) * items_per_page;
    }
...
t_data_slice<CTX_T>::get_row_path_extra(t_uindex ridx) const {
    auto config = m_ctx->get_config();
    auto paginationspec = config.get_paginationspec();
    // Check enable pagination or not
    if (paginationspec.enable_pagination()) {
    ...
    return m_ctx->unity_get_row_path_extra(ridx);
}

template <typename CTX_T>
std::vector<t_tscalar>
t_data_slice<CTX_T>::get_row_header(t_uindex ridx) const {
    auto config = m_ctx->get_config();
    auto paginationspec = config.get_paginationspec();
    // Check enable pagination or not
    if (paginationspec.enable_pagination()) {
    ...
    return m_ctx->unity_get_row_header(ridx);
}
...
template <typename CTX_T>
const std::vector<std::vector<t_tscalar>>&
t_data_slice<CTX_T>::get_short_column_names() const {
    return m_short_column_names;
}
