...
t_lstore::reserve_impl(t_uindex capacity, bool allow_shrink) {
    ...
    // GAB: memory optim: don't use the resize factor in this reserve() function, as it
    // unecessarilly increase the storage size for the use cases where we know in advance the data size
    // Instead, the resize factor is only used in the push_back() method when we find we need to grow
    // the storage
    capacity = 4 * std::uint64_t(ceil(double(capacity * 1./* m_resize_factor */) / 4));
...
t_lstore::push_back(const void* ptr, t_uindex len) {
    ...
    if (m_size + len >= m_capacity) {
        reserve(static_cast<t_uindex>(
            m_size + len)*m_resize_factor);
        // GAB: memory optim: Let's use the resize factor here, and NOT in the reserve() function as it was done initially,
        // as it is better to have reserve() to allocate exactly the number of requested items
    }
