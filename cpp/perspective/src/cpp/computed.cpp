...
t_computed_column::apply_computation(
    const std::vector<t_computed_column_input>& table_columns,
    t_uindex row_count,
    std::shared_ptr<t_column> output_column,
    t_computation computation) {
    std::uint32_t end = row_count;
    ...
    std::cout << "apply_computation---" << end << "---" << arity << std::endl;
    switch (arity) {
        case 1: {
    ...
    for (t_uindex idx = 0; idx < end; ++idx) {
        ...
        for (t_uindex x = 0; x < arity; ++x) {
            t_tscalar t = table_columns[x].getValue(idx);
            if (!t.is_valid()) {
        ...
        if (computation.m_return_type == DTYPE_STR) {
            ...
            if (rval.is_error()) {
                std::cout << "-------rval.error-----" << idx << std::endl;
                output_column->set_scalar(idx, rval);
            } else if (!rval.is_valid() || rval.is_none()) {
                std::cout << "-------rval.invalid-----" << idx << std::endl;
                output_column->clear(idx);
            } else {
                output_column->set_scalar(idx, rval);
            }
...
t_computed_column::reapply_computation(
    const std::vector<t_computed_column_input>& table_columns,
    const std::vector<t_computed_column_input>& flattened_columns,
    ...
    for (t_uindex idx = 0; idx < end; ++idx) {
        ...
        for (t_uindex cidx = 0; cidx < arity; ++cidx) {
            t_tscalar arg = flattened_columns[cidx].getValue(idx);
            ...
            if (!arg.is_valid()) {
                ...
                if (should_unset) {
                    ...
                    arg = table_columns[cidx].getValue(ridx);
                }
            }
...
t_computed_column::computed_functions = {
    ...
    {"concat_comma", {
        {"name", "concat_comma"},
        {"label", "concat_comma(value1, value2)"},
...
void t_computed_column_input::setScalar(const std::string& value, const std::string& type) {
    if (type == "string") {
        m_type = type;
        m_scalar.set(value);
    }
...
void t_computed_column_input::setColumn(std::shared_ptr<t_column> column) {
    m_column = column;
    m_type = "column";
}

t_tscalar t_computed_column_input::getValue(int idx) const {
    if (m_type == "string") {
        return m_scalar;
    }
        ...
        return m_column->get_scalar(idx);
    }
    else {
        return mknone();
    }
