...
t_dtree::t_dtree(const std::string& dirname, t_dssptr ds, const std::vector<t_pivot>& pivots,
...
t_dtree::init() {
    ...
    m_leaves = t_column(DTYPE_UINT64, false, leaf_args, DEFAULT_CAPACITY, DATA_FORMAT_NUMBER);
    m_leaves.init();
    ...
    m_values[0] = t_column(DTYPE_STR, true, leaf_args, DEFAULT_CAPACITY, DATA_FORMAT_TEXT);
    m_values[0].init();
    ...
    for (t_uindex idx = 0, loop_end = m_pivots.size(); idx < loop_end; ++idx) {
        ...
        auto pivot_name = m_pivots[idx].pivot_name();
        ...
        bool has_sortby = siter != m_sortby_columns.end() && siter->second != colname;
        m_has_sortby[idx + 1] = has_sortby;
        // 23/08/2019: allow aggregate multiple aggregations for same column name
        //std::string sortby_column = has_sortby ? siter->second : colname;
        std::string sortby_column = has_sortby ? siter->second : pivot_name;
        m_sortby_dpthcol.push_back(sortby_column);
        // 23/08/2019: allow aggregate multiple aggregations for same column name
        //t_dtype dtype = m_ds->get_dtype(colname);
        t_dtype dtype = m_ds->get_dtype(pivot_name);
        // 23/08/2019: allow aggregate multiple aggregations for same column name
        //t_dataformattype dftype = m_ds->get_data_format_type(colname);
        t_dataformattype dftype = m_ds->get_data_format_type(pivot_name);
        m_values[idx + 1] = t_column(dtype, true, leaf_args, DEFAULT_CAPACITY, dftype);
        m_values[idx + 1].init();
    }
...
void
t_dtree::check_pivot(const t_filter& filter, t_uindex level, t_config& config, t_mask &dt_msk, const std::vector<t_sortspec>& ctx_sortby) {
    if (level <= m_levels_pivoted)
        return;

    PSP_VERBOSE_ASSERT(level <= m_pivots.size() + 1, "Erroneous level passed in");

    pivot(filter, level, config, dt_msk, ctx_sortby);
}

void
t_dtree::pivot(const t_filter& filter, t_uindex level, t_config& config, t_mask &dt_msk, const std::vector<t_sortspec>& ctx_sortby) {
    if (level <= m_levels_pivoted)
        return;
    ...
    // Get sort map for each level base on column value
    std::map<t_depth, t_sortspec> sort_map;
    for (const auto& s : ctx_sortby) {
        if (s.m_sortby_index == t_index(-1)) {
    ...
    for (t_uindex pidx = m_levels_pivoted; pidx < level; pidx++) {
        ...
        if (pidx == 0) {
            ...
            fill_dense_tnode(root, nidx, nidx, 1, 0, 0, nrows, true);
            nidx++;

            m_values[0].push_back(std::string("Grand Aggregate"));
            m_levels.push_back(std::pair<t_uindex, t_uindex>(nbidx, neidx));
        } else {
            // Get default sort type and limit for current level
            t_sorttype sort_type = SORTTYPE_ASCENDING;
            t_index sort_limit = -1;
            t_limit_type limit_type = LIMIT_TYPE_ITEMS;
            // Check sort exists in sort map
            auto sit = sort_map.find(pidx - 1);
            if (sit != sort_map.end()) {
                auto s = sit->second;
                // Get sort and limit from sort item
                sort_type = s.m_sort_type;
                sort_limit = s.m_limit;
                limit_type = s.m_limit_type;
            }
            const t_pivot& pivot = m_pivots[pidx - 1];
            std::string tbl_colname = pivot.colname();
            auto top_n_filters = config.get_top_n_filters(tbl_colname);
            if (top_n_filters.size() > 0) {
                for (t_index idx = 0, fsize = top_n_filters.size(); idx < fsize; ++idx) {
                    sort_limit = t_index(top_n_filters[idx].m_bag[0].to_int64());
                    limit_type = str_to_limit_type(top_n_filters[idx].m_bag[1].to_string());
                }
            }
            // 23/08/2019: allow aggregate multiple aggregations for same column name
            //std::string pivot_colname = pivot.colname();
            std::string pivot_colname = pivot.pivot_name();
            ...
            switch (piv_dtype) {
                case DTYPE_STR: {
                    next_neidx = t_pivot_processor<DTYPE_STR>()(
                        pivcol, &m_nodes, &(m_values[pidx]), &m_leaves, nbidx, neidx, mask, config, dt_msk, sort_type, sort_limit, limit_type);
                } break;
                case DTYPE_INT64: {
                ...
                case DTYPE_DECIMAL: {
                    next_neidx = t_pivot_processor<DTYPE_DECIMAL>()(
                        pivcol, &m_nodes, &(m_values[pidx]), &m_leaves, nbidx, neidx, mask, config, dt_msk, sort_type, sort_limit, limit_type);
                } break;
                case DTYPE_LIST_STR: {
                    next_neidx = t_pivot_processor<DTYPE_LIST_STR>()(
                        pivcol, &m_nodes, &(m_values[pidx]), &m_leaves, nbidx, neidx, mask, config, dt_msk, sort_type, sort_limit, limit_type);
                } break;
                case DTYPE_LIST_BOOL: {
                    next_neidx = t_pivot_processor<DTYPE_LIST_BOOL>()(
                        pivcol, &m_nodes, &(m_values[pidx]), &m_leaves, nbidx, neidx, mask, config, dt_msk, sort_type, sort_limit, limit_type);
                } break;
                case DTYPE_LIST_FLOAT64: {
                    next_neidx = t_pivot_processor<DTYPE_LIST_FLOAT64>()(
                        pivcol, &m_nodes, &(m_values[pidx]), &m_leaves, nbidx, neidx, mask, config, dt_msk, sort_type, sort_limit, limit_type);
                } break;
                case DTYPE_LIST_INT64: {
                    next_neidx = t_pivot_processor<DTYPE_LIST_INT64>()(
                        pivcol, &m_nodes, &(m_values[pidx]), &m_leaves, nbidx, neidx, mask, config, dt_msk, sort_type, sort_limit, limit_type);
                } break;
                case DTYPE_LIST_DATE: {
                    next_neidx = t_pivot_processor<DTYPE_LIST_DATE>()(
                        pivcol, &m_nodes, &(m_values[pidx]), &m_leaves, nbidx, neidx, mask, config, dt_msk, sort_type, sort_limit, limit_type);
                } break;
                case DTYPE_LIST_TIME: {
                    next_neidx = t_pivot_processor<DTYPE_LIST_TIME>()(
                        pivcol, &m_nodes, &(m_values[pidx]), &m_leaves, nbidx, neidx, mask, config, dt_msk, sort_type, sort_limit, limit_type);
                } break;
                case DTYPE_LIST_DURATION: {
                    next_neidx = t_pivot_processor<DTYPE_LIST_DURATION>()(
                        pivcol, &m_nodes, &(m_values[pidx]), &m_leaves, nbidx, neidx, mask, config, dt_msk, sort_type, sort_limit, limit_type);
                } break;
    ...
    /*for (t_uindex idx = 0, nsize = m_nodes.size(); idx < nsize; ++idx) {
        std::cout << "node at index " << idx << " is " << m_nodes[idx] << std::endl;
    }*/
}
...
t_dtree::pprint(const t_filter& filter) const {
    ...
    for (auto idx : dfs()) {
        ...
        std::cout << get_value(filter, idx) << " idx => " << node->m_idx << " pidx => "
                  ...
                  << node->m_nleaves << " show => " << node->m_show << std::endl;
    }
}
...
t_tscalar
t_dtree::_get_value(const t_filter& filter, t_index nidx, bool sort_value) const {
    ...
    if (sort_value || nidx == 0) {
        ...
        return col.get_pivot_scalar(scalar_idx);
    } else {
