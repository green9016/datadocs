# ICU CMakefile
# Target ICU version: 64-2
#
# GAB: this file have been inspired from ICU integration to Google Skia Emscripten port.
# See https://github.com/blockspacer/skia-opengl-emscripten/blob/master/icu.cmake
#
# This will certainly need to be tweaked according to the real needs of c-ingest
# Also, ICU needs a "data" file to work. Data files can be quite huge (20MB by default!)
# But they can be filtered out to be smaller. I've extracted a data file from Chromium project that is 500kb in size
# It is located in 'deps/icu-init/res' subfolder. It should be loaded at runtime, a loader is implemented by 'icu-init'
# project.
#
set(ICU_FULL_DIR ${CMAKE_CURRENT_BINARY_DIR}/icu/icu4c/)

# Options to customize ICU build. All off by default
set(DISABLE_COLLATION FALSE CACHE BOOL "UCONFIG_NO_COLLATION")
set(DISABLE_FORMATTING FALSE CACHE BOOL "UCONFIG_NO_FORMATTING")


# Define ICU sources: 'i18n' and 'common'
# find source/i18n -maxdepth 1 ! -type d | egrep '\.(c|cpp)$' |  sort | sed "s/^\(.*\)$/ '\1',/"
# find source/common -maxdepth 1 ! -type d | egrep '\.(c|cpp)$' |  sort | sed "s/^\(.*\)$/ '\1',/"
set(ICU_SOURCES
    ${ICU_FULL_DIR}source/stubdata/stubdata.cpp
    # I18N_SRC_START
...
# ICU library target property => C++ 17
...
########## Public compilation options and definitions
...
# options kept: normalization, collation, formatting, break iteration, UCHAR_TYPE=char16_t
...
list(APPEND ICU_PUBLIC_DEFINES
    U_ENABLE_DYLOAD=0                 # no dynamic libraries
    U_CHECK_DYLOAD=0                  # no dynamic libraries
    U_CHARSET_IS_UTF8=1               # by default, char* is assumed to be UTF8
    UCONFIG_NO_FILE_IO=1              # no file IO
    UCONFIG_ONLY_HTML_CONVERSION=1    # only support encodings as defined by HTML standard:
    UCONFIG_NO_IDNA=1                 # no international domain names handling
    UCONFIG_NO_REGULAR_EXPRESSIONS=1  # no unicode regexp
    UCONFIG_NO_TRANSLITERATION=1      # no transliteration
    )
...
if (MINGW)
  # GAB: hack: mingw might incorrectly detect the windows version, leading to compilation fail
  target_compile_definitions(icu PRIVATE
          ${ICU_PUBLIC_DEFINES} _WIN32_WINNT=0xA000 # force at least Windows 10
          )
endif()

###### Private compile definitions and options
...
target_compile_options(icu PRIVATE
  ...
  target_compile_options(icu PRIVATE /source-charset:utf-8)
endif()
