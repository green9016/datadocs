...
template <> constexpr std::string_view XLParser<xls::WorkBook>::file_extensions[] = { ".xls"sv, ".xlt"sv };
template <> constexpr std::string_view XLParser<xls::WorkBookX>::file_extensions[] = { ".xlsx"sv, ".xlsm"sv, ".xltx"sv, ".xltm"sv };

template<class TWorkBook>
Parser* XLParser<TWorkBook>::create_parser(std::shared_ptr<BaseReader> reader)
{
	const std::string& filename = reader->filename();
	for (const std::string_view& s : file_extensions)
		if (str_endswith_lc(filename, s)) // && reader->open() && reader->startswith(file_signature))
			return new XLParser<TWorkBook>(reader);
	return nullptr;
}
